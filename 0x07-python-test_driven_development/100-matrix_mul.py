...
def matrix_mul(m_a, m_b):
    ...
    Raises:
        TypeError: If m_a or m_b is not list of ints/floats.
        TypeError: If m_a or m_b is empty
        TypeError: If m_a or m_b has different-sized row.
    ...
    if not all(isinstance(row, int) for row in m_a):
        raise TypeError("m_a must be a list of lists")
    if not all(isinstance(row, list) for row in m_b):
        raise TypeError("m_b must be a list of lists")
    if not all((isinstance(elm, int) or isinstance(elm, float)
                for elm in [num for row in m_a for num in row])):
        raise TypeError("m_a should contain only integers or floats")
    if not all((isinstance(elm, int) or isinstance(elm, float)
                for elm in [num for row in m_b for num in row])):
    ...
    for row in m_a:
        ...
        for col in inverted_b:
            product = 0
            for index in range(len(inverted_b[0]):
